...
void Armor::show(const cv::Mat &ref, const std::string &win) {
    ...
    cv::imshow(win, ref);
    cv::waitKey(0);
    // Destroy only if a window exists with that name; destroyAllWindows is safer across platforms
...
cv::Mat Armor::get_mask(const cv::Mat &img /*, const std::string& team*/) {
    cv::Mat gray, mask;
...
std::vector<cv::Mat> Armor::process(const std::string &img_path, cv::Mat *out_digit_roi, bool visualize) {
    ...
    std::sort(order.begin(), order.end(),
              [&](int a, int b) { return bbs[a].x < bbs[b].x; });

    const int W_img = img.cols;
    const int max_dx = std::max(20, static_cast<int>(max_dx_ratio * W_img));
    ...
    for (size_t oi = 0; oi < order.size(); ++oi) {
        ...
        for (size_t oj = oi + 1; oj < order.size(); ++oj) {
            ...
            const double hr = static_cast<double>(Bj.height + 1e-6) /
                              static_cast<double>(Ai.height + 1e-6);
            if (hr < height_ratio_lo || hr > height_ratio_hi) continue;
            ...
            if (approx.size() == 4) {
                ...
                auto wh   = resize_from_quad(quad);
                int Wp = wh.first, Hp = wh.second;
                ...
                std::vector<cv::Point2f> quad_vec(quad.begin(), quad.end());
                cv::Mat H = cv::getPerspectiveTransform(quad_vec, dst);

                if (!cv::checkRange(H, true, nullptr, -FLT_MAX, FLT_MAX)) continue;

                cv::Mat patch;
                cv::warpPerspective(img, patch, H, cv::Size(Wp, Hp),
                ...
                cv::perspectiveTransform(boxA, boxA_w, H);
                cv::perspectiveTransform(boxB, boxB_w, H);
                ...
                const int Hh = patch.rows, Ww = patch.cols;

                int pad_x = std::max(pad_x_min, static_cast<int>(0.000001 * Ww));
                int x1 = std::max(0, L.x + L.width - pad_x);
                int x2 = std::min(Ww, R.x + pad_x);
                int y1 = 0, y2 = Hh;

                int min_w = std::max(12, static_cast<int>(min_w_ratio * Ww));
                int w = x2 - x1;
                if (w >= min_w) {
                    int cx = (x1 + x2) / 2;
                    int w_keep = std::max(min_w, static_cast<int>(w * keep_ratio));
                    x1 = std::max(0, cx - w_keep / 2);
                    x2 = std::min(Ww, x1 + w_keep);
                    ...
                    if (x2 > x1) {
                        ...
                        if (visualize) {
                            ...
                            cv::imshow("patch", vis);
                            cv::imshow("digit_roi", digit_roi);
                            cv::waitKey(0);
                            cv::destroyAllWindows();
                        }

                        if (out_digit_roi) {
                            *out_digit_roi = std::move(digit_roi);
                        }
                        mat.push_back(patch.clone());
                    }
                }
            }
        }
